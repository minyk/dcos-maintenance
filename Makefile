...
windows:
	docker run --rm -e "GOOS=windows" -e "GOARCH=amd64" -v $(PWD):/usr/src/github.com/minyk/dcos-maintenance -w /usr/src/github.com/minyk/dcos-maintenance golang:1.11 go build -ldflags="-s -w ${GO_LDFLAGS}" -v -o build/dcos-maintenance-windows.exe
